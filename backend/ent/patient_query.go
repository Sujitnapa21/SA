...
func (pq *PatientQuery) QueryStatus() *StatusQuery {
...
func (pq *PatientQuery) WithStatus(opts ...func(*StatusQuery)) *PatientQuery {
	query := &StatusQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PatientQuery) sqlAll(ctx context.Context) ([]*Patient, error) {
	...
	if pq.withEmployee != nil || pq.withStatus != nil || pq.withBloodtype != nil || pq.withGender != nil || pq.withNametitle != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withStatus; query != nil {
		...
		nodeids := make(map[int][]*Patient)
		for i := range nodes {
			if fk := nodes[i].status_patient; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "status_patient" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := pq.withNametitle; query != nil {
		...
		nodeids := make(map[int][]*Patient)
		for i := range nodes {
			if fk := nodes[i].name_title_patient; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "name_title_patient" returned %v`, n.ID)
			}
			for i := range nodes {
