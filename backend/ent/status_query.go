...
func (sq *StatusQuery) QueryPatient() *PatientQuery {
...
func (sq *StatusQuery) WithPatient(opts ...func(*PatientQuery)) *StatusQuery {
	query := &PatientQuery{config: sq.config}
...
func (sq *StatusQuery) sqlAll(ctx context.Context) ([]*Status, error) {
	...
	if query := sq.withPatient; query != nil {
		...
		query.Where(predicate.Patient(func(s *sql.Selector) {
			s.Where(sql.InValues(status.PatientColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.status_patient
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "status_patient" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "status_patient" returned %v for node %v`, *fk, n.ID)
			}
